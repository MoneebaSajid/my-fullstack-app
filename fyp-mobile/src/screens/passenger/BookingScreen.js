...
//   const [distanceFetching, setDistFetch]  = useState(false);
//   const [errors, setErrors]               = useState({});

//   // ── Local text states (keyboard fix) ──
...
//   // ── GPS coords for distance calc ──
//   const [pickupCoords,  setPickupCoords]  = useState(null);
//   const [dropoffCoords, setDropoffCoords] = useState(null);

//   // ── DateTime ──
//   const [startDate, setStartDate] = useState(new Date());
//   const [endDate,   setEndDate]   = useState(null); // auto-calculated
...
//   // ── Auto calculate distance when both coords available ──
//   useEffect(() => {
...
//     if (pickup.length > 2 && dropoff.length > 2 && km > 0 && startDate) {
...
//       setEndDate(null);
//     }
//   }, [pickupText, dropoffText, estimatedKm, rateType, startDate]);
...
//     setEstimatedKm(String(rounded));
//   };

//   // ── Auto calculate end time based on distance + avg speed ──
//   const calculateAutoEndTime = (km) => {
//     if (!startDate || km <= 0) return;
//     const tripHours  = km / AVG_SPEED_KMH;
//     const bufferHrs  = rateType === 'daily' ? 12 : Math.max(1, Math.ceil(tripHours));
//     const end        = new Date(startDate.getTime() + bufferHrs * 60 * 60 * 1000);
//     setEndDate(end);
//   };
...
//       const farePerDay    = farePerHour * 12;
//       const perKmRate     = 25;
//       const gst           = 0.05;
...
//       const tripHours  = distKm / AVG_SPEED_KMH;
//       const billedHrs  = rateType === 'daily' ? 12 : Math.max(1, Math.ceil(tripHours));
//       const billedDays = Math.ceil(billedHrs / 12);

//       let baseFare     = 0;
//       let durationLabel = '';

//       if (rateType === 'hourly') {
//         baseFare      = billedHrs * farePerHour;
//         durationLabel = `${billedHrs} hr(s) × Rs.${farePerHour}`;
//       } else if (rateType === 'daily') {
//         baseFare      = billedDays * farePerDay;
//       } else {
//         baseFare      = distKm * perKmRate;
//         durationLabel = `${distKm} km × Rs.${perKmRate}/km`;
//       }

//       const distCharge = rateType !== 'per_km' ? Math.round(distKm * perKmRate) : 0;

//       // Driver fee — experience based
//       let driverFee = 0;
//       if (bookingType === 'with-driver') {
//         const exp  = parseInt(selectedDriver?.experience_years) || 0;
//         const rate = exp >= 5 ? 1000 : exp >= 3 ? 700 : 500;
//         const minF = exp >= 5 ? 4000 : exp >= 3 ? 3000 : 2000;
//         driverFee  = Math.max(billedHrs * rate, minF);
//       }

//       const subtotal  = Math.round(baseFare) + distCharge + driverFee;
//       const taxAmt    = Math.round(subtotal * gst);
...
//         if (field === 'pickup') {
//           setPickupText(full);
//           pickupRef.current = full;
//           setPickupCoords(coords);
//         } else {
//           setDropoffText(full);
//           dropoffRef.current = full;
...
//     date
//       ? date.toLocaleString('en-PK', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
//       : '--';
...
//       errs.dropoff = 'Pickup and dropoff cannot be same!';
//     if (!estimatedKm || isNaN(km) || km <= 0 || km > 2000)
//       errs.distance = 'Distance must be between 1 and 2000 km!';
//     if (!endDate)
//       errs.time = 'End time not calculated yet. Please fill all fields!';
...
//     if (!validate()) {
//       Alert.alert('Incomplete', 'Please fix the errors below!');
//       return;
//     }
...
//           rate_type:            rateType,
//           self_pickup_location: pickup,
//           onsite_location:      dropoff,
//           estimated_distance:   parseFloat(estimatedKm),
//           special_requests:     specialReq,
//         });
//       }
...
//         `Base Fare: Rs. ${fare?.base_fare || 0}\n` +
//         `Distance (${fare?.distance_km} km): Rs. ${fare?.distance_charge || 0}\n` +
//         (fare?.driver_fee > 0 ? `Driver Fee: Rs. ${fare.driver_fee}\n` : '') +
//         `Tax (5%): Rs. ${fare?.tax_amount || 0}\n` +
//         `─────────────────\n` +
//         `TOTAL: Rs. ${response.data.total_amount}`,
//         [
//           {
//             text: 'Pay Now 💳',
...
//   const LocationField = ({ label, field, type, setType }) => {
//     const isPickup = field === 'pickup';
//     const text     = isPickup ? pickupText : dropoffText;
//     const setText  = isPickup ? setPickupText : setDropoffText;
//     const ref      = isPickup ? pickupRef : dropoffRef;
//     const errKey   = isPickup ? 'pickup' : 'dropoff';
...
//               value={text}
//               onChangeText={(val) => {
//                 // ONLY local state — no form, no re-render of parent
//                 setText(val);
//                 ref.current = val;
//                 if (errors[errKey]) setErrors(p => ({ ...p, [errKey]: null }));
//                 // If manual typing, clear coords so distance resets
//                 if (type === 'manual') {
//                   if (isPickup)  setPickupCoords(null);
//                   else           setDropoffCoords(null);
//                   // Debounce: update km manually if they type
//                 }
//               }}
...
//               blurOnSubmit={false}
//               returnKeyType="next"
//               // KEY FIX: No onBlur that triggers state update!
//             />
//           </View>
...
//         {errors[errKey] && <Text style={styles.errorTxt}>⚠️ {errors[errKey]}</Text>}
//       </View>
//     );
//   };
...
//                 Rs. {Math.round(parseFloat(vehicle.fare_per_hour) * 12)}
//               </Text>
//             </View>
//             <View style={styles.priceDivider} />
//             <View style={styles.priceItem}>
//               <Text style={styles.priceLabel}>Per KM</Text>
//               <Text style={styles.priceVal}>Rs. 25</Text>
//             </View>
//           </View>
//         </View>
...
//           {/* Location Fields */}
//           <LocationField label="Pickup Location"  field="pickup"  type={pickupType}  setType={setPickupType} />
//           <LocationField label="Dropoff Location" field="dropoff" type={dropoffType} setType={setDropoffType} />

//           {/* Distance — Auto or Manual */}
//           <Text style={styles.label}>
//             Estimated Distance (KM) *
//             {distanceFetching && <Text style={{ color: COLORS.light }}> Calculating...</Text>}
//           </Text>
...
//                   : 'Enter distance (e.g. 25)'
//               }
//               placeholderTextColor={COLORS.textMuted}
//               value={estimatedKm}
//               onChangeText={(val) => {
//                 const cleaned = val.replace(/[^0-9.]/g, '');
//                 setEstimatedKm(cleaned);
...
//           </View>
//           {errors.distance && <Text style={styles.errorTxt}>⚠️ {errors.distance}</Text>}
...
//                 Based on {estimatedKm} km @ avg {AVG_SPEED_KMH} km/h
//               </Text>
//             </View>
//           )}
...
//                 <Text style={styles.fareVal}>Rs. {farePreview.base_fare}</Text>
//               </View>
//               <Text style={styles.fareSub}>{farePreview.duration_label}</Text>
//               {farePreview.distance_charge > 0 && (
//                 <View style={styles.fareRow}>
//                   <Text style={styles.fareLabel}>Distance ({farePreview.distance_km} km × Rs.25)</Text>
//                   <Text style={styles.fareVal}>Rs. {farePreview.distance_charge}</Text>
//                 </View>
//               )}
...
//                           <Text style={styles.driverFeeText}>Rs.{rate}/hr</Text>
//                         </View>
//                         <Text style={styles.driverMinFee}>Min: Rs.{minF}</Text>
...
//   typeBadge:     { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(46,134,222,0.15)', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, borderWidth: 1, borderColor: COLORS.glassBorder, marginBottom: 14 },
...
export default function BookingScreen({ route, navigation }) {
  ...
  const computeFare = (km) => {
    try {
      const farePerHour = parseFloat(vehicle.fare_per_hour) || 0;
      const farePerDay = farePerHour * HOURS_PER_DAY;
      const perKmRate = parseFloat(vehicle.fare_per_km) || 0;
      const gst = GST_RATE;
      const distKm = parseFloat(km) || 0;

      const tripHrs = distKm / AVG_SPEED_KMH;
      const billedHrs = rateType === 'daily'
        ? Math.max(HOURS_PER_DAY, Math.ceil(tripHrs / HOURS_PER_DAY) * HOURS_PER_DAY)
        : Math.max(1, Math.ceil(tripHrs));
      const billedDays = Math.max(1, Math.ceil(billedHrs / HOURS_PER_DAY));

      let baseFare = 0, durationLabel = '';
      if (rateType === 'hourly') {
        baseFare = billedHrs * farePerHour;
        durationLabel = `${billedHrs} hr(s) × Rs.${farePerHour}`;
      } else if (rateType === 'daily') {
    ...
    try {
      ...
      Alert.alert(
        '✅ Booking Successful!',
          `Receipt: ${response.data.receipt_number}\n\n` +
          `Base Fare:    Rs. ${fare?.base_fare || 0}\n` +
          `Distance:     ${fare?.distance_km || 0} km\n` +
          (fare?.driver_fee > 0 ? `Driver Fee:   Rs. ${fare.driver_fee}\n` : '') +
          `Tax (5%):     Rs. ${fare?.tax_amount || 0}\n` +
          `─────────────────────\n` +
          `TOTAL:        Rs. ${response.data.total_amount}`,
        ...
        <View style={styles.headerCard}>
          ...
          <View style={styles.pricingRow}>
            ...
            <View style={styles.priceItem}>
              <Text style={styles.priceLabel}>Per Day (12hr)</Text>
              <Text style={styles.priceVal}>
                Rs. {Math.round(parseFloat(vehicle.fare_per_hour || 0) * HOURS_PER_DAY)}
              </Text>
            </View>
            <View style={styles.priceDivider} />
            <View style={styles.priceItem}>
        ...
        <View style={styles.card}>
          ...
          {farePreview && (
            <View style={styles.fareCard}>
              ...
              <Text style={styles.fareSub}>{farePreview.duration_label}</Text>
              <Text style={styles.fareSub}>Distance used: {farePreview.distance_km} km</Text>

              {farePreview.distance_charge > 0 && (
                <View style={styles.fareRow}>
                  <Text style={styles.fareLabel}>
                    Distance ({farePreview.distance_km} km × Rs.25)
                  </Text>
                  <Text style={styles.fareVal}>Rs. {farePreview.distance_charge}</Text>
                </View>
              )}
